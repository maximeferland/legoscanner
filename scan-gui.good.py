            ...
            if s.get("forced_image_path"):
                ...
                if s.get("dual_cam") and s.get("cam2_idx", -1) >= 0:
                    ...
                    try:
                        ...
                        backends = ([known2, cv2.CAP_DSHOW, cv2.CAP_MSMF] if known2
                                    else [cv2.CAP_DSHOW, cv2.CAP_MSMF])
                        cap2 = None
                        for backend in dict.fromkeys(backends):
                            c = cv2.VideoCapture(cam2_idx, backend)
                            if c.isOpened():
                                cap2 = c; break
                            c.release()
                        if cap2:
...
class ScanStation(QMainWindow):
    ...
    def _build_ui(self):
        ...
        gl.addWidget(QLabel("IP stream:"), 2, 0)
    ...
    def _update_bulk_bar(self):
        ...
        _action_btns = [self._bulk_set_qty, self._bulk_set_med, self._bulk_set_med15,
                        ...
                        self._bulk_recolor, self._bulk_del, self._bulk_agg]
    ...
    def _update_total_value(self):
        ...
        all_qty   = sum(r.get("qty", 1) for r in all_rows)
        all_total = sum((r.get("price") or 0) * r.get("qty", 1) for r in all_rows)
        ...
        if sel:
            sel_lots  = len(sel)
            sel_qty   = sum(r.get("qty", 1) for r in sel)
            sel_total = sum((r.get("price") or 0) * r.get("qty", 1) for r in sel)
