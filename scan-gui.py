...
class ScanWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Color source mode (scanner vs Brickognize vs merge)
            try:
                mode_val = self.color_mode_combo.currentData() if hasattr(self, "color_mode_combo") else "merge"
                if mode_val not in ("merge", "brickognize", "scan"):
                    mode_val = "merge"
                if mode_val != "merge":
                    args += ["--color-mode", mode_val]
            except Exception:
                pass
            if s.get("bg_color"):
                r,g,b = s["bg_color"]
...
class ScanStation(QMainWindow):
    ...
    def __init__(self):
        ...
        self._iphone_dx    = 0
        self._iphone_dy    = 0
        self._iphone_scale = 1.0   # zoom-out factor: >1 = iPhone crop covers more area (shrinks apparent box)
    ...
    def _build_ui(self):
        ...
        # Color source mode (Scanner vs Brickognize vs smart merge)
        gl.addWidget(QLabel("Color source:"), 11, 0)
        self.color_mode_combo = QComboBox()
        self.color_mode_combo.addItem("Smart merge (B vs S)", "merge")
        self.color_mode_combo.addItem("Brickognize only", "brickognize")
        self.color_mode_combo.addItem("Scanner only", "scan")
        self.color_mode_combo.setCurrentIndex(0)
        gl.addWidget(self.color_mode_combo, 11, 1)

        # Grid mode
        self.grid_check = QCheckBox("Fixed grid mode")
        self.grid_check.setToolTip("Use when parts are arranged in a regular grid")
        gl.addWidget(self.grid_check, 13, 0, 1, 2)
        grid_sub = QWidget()
        ...
        gl.addWidget(grid_sub, 14, 0, 1, 2)
        ...
        gl.addWidget(QLabel("Background:"), 15, 0)
        bg_sub = QWidget()
        bg_h = QHBoxLayout(bg_sub); bg_h.setContentsMargins(0,0,0,0); bg_h.setSpacing(0)
        self._bg_rgb = None

        self.bg_swatch = QLabel("🖱 Left-click preview or here to set background")
        ...
        self.bg_swatch.setText("Left-click preview or here to set background")
        ...
        def _bg_mouse(event):
            if event.button() == Qt.LeftButton:
                ...
                _clear_bg_color()  # right-click swatch = clear
        ...
        self.shadow_swatch = QLabel("🌑 Right-click preview to set shadow")
        ...
        def _update_shadow_swatch():
            if self._shadow_rgb:
                ...
                self.shadow_swatch.setStyleSheet(
                    f"background:{CARD_BG};color:{TEXT_DIM};border:1px solid {BORDER};"
                    f"border-radius:3px;font-size:10px;padding:0 6px;")
                self.shadow_swatch.setText("🌑 Right-click preview to set shadow")
        ...
        self.preview_label.setToolTip("Left-click → set background color\nRight-click → set shadow color")
    ...
    def _load_config(self):
        if CFG_PATH.exists():
            try:
                ...
                try:
                    self._iphone_scale = float(cfg.get("iphone_scale", 1.0))
                    if not (0.1 <= self._iphone_scale <= 10.0):
                        self._iphone_scale = 1.0
                except Exception:
                    self._iphone_scale = 1.0
    ...
    def _save_iphone_offset(self):
        """Persist current iPhone offset and scale into station.cfg."""
        try:
            cfg = {}
            if CFG_PATH.exists():
                try:
            ...
            cfg["iphone_scale"]  = round(float(getattr(self, "_iphone_scale", 1.0)), 4)
            CFG_PATH.write_text(json.dumps(cfg, indent=2))
        except Exception:
            pass
    ...
    def _calibrate_iphone_offset(self, src, iph_path, bbox):
        """Interactive calibration: align iPhone crop with DroidCam crop.

        Controls:
          • Drag the iPhone (right) image to pan the offset
          • Scale slider: enlarge/shrink the iPhone crop region so boxes match
          • Scroll wheel = ±1 px vertical;  Shift+Scroll = horizontal
          • Arrow buttons = ±5 px nudge
        """
        try:
            from PyQt5.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout,
                                         QLabel as _QL, QPushButton, QSlider,
                                         QSizePolicy)
            from PyQt5.QtGui import QPixmap as _QP, QCursor
            from PyQt5.QtCore import Qt as _Qt
            from PIL import Image as _PIL

            PREVIEW_SIZE = 500   # px — big enough for comfortable dragging

            # ── Draggable label ────────────────────────────────────────────────
            class _DraggableLabel(_QL):
                """QLabel supporting drag-to-pan and scroll-to-nudge."""
                def __init__(self, nudge_fn, get_scale_fn):
                    super().__init__()
                    self._nudge = nudge_fn
                    self._get_scale = get_scale_fn  # returns current iphone scale
                    self._drag_origin = None
                    self._drag_dx = 0.0
                    self._drag_dy = 0.0
                    self.setAlignment(_Qt.AlignCenter)
                    self.setCursor(QCursor(_Qt.OpenHandCursor))
                ...
                def mousePressEvent(self, ev):
                    if ev.button() == _Qt.LeftButton:
                        ...
                        self._drag_dx = 0.0
                        self._drag_dy = 0.0
                        self.setCursor(QCursor(_Qt.ClosedHandCursor))
                    super().mousePressEvent(ev)

                def mouseMoveEvent(self, ev):
                    if self._drag_origin is not None and (ev.buttons() & _Qt.LeftButton):
                        delta  = ev.pos() - self._drag_origin
                        self._drag_origin = ev.pos()
                        # Convert label-px → iPhone-px:
                        # displayed pixmap covers PREVIEW_SIZE px of label space;
                        # the crop window in iPhone-px = (crop_dim * iphone_scale).
                        # We approximate by scaling by get_scale() so dragging feels
                        # proportional regardless of zoom level.
                        sc = self._get_scale()
                        self._drag_dx += delta.x() * sc
                        self._drag_dy += delta.y() * sc
                        ddx = int(self._drag_dx)
                        ddy = int(self._drag_dy)
                        if ddx or ddy:
                            self._nudge(ddx, ddy)
                            self._drag_dx -= ddx
                            self._drag_dy -= ddy
                ...
                def wheelEvent(self, ev):
                    steps = ev.angleDelta().y() // 40
                    if not steps:
                        steps = 1 if ev.angleDelta().y() > 0 else -1
            ...
            if (wd > hd and wi < hi) or (wd < hd and wi > hi):
                ...
                wi, hi = iph_img.size
            # Base coordinate scale: how many iPhone px per DroidCam px
            sx_base = wi / float(wd)
            sy_base = hi / float(hd)

            # Static scan crop (reference, never changes)
            scan_crop = scan_img.crop((x1, y1, x2, y2))
            tmp_scan = Path.cwd() / "_tmp_scan_crop_preview.jpg"
            scan_crop.save(str(tmp_scan), "JPEG", quality=85)

            # Mutable state
            dx    = self._iphone_dx
            dy    = self._iphone_dy
            scale = getattr(self, "_iphone_scale", 1.0)   # zoom of the crop window

            def _get_scale():
                return scale

            def _update_iphone():
                """Re-crop iPhone image and refresh the right label."""
                # Effective scale: base * user_scale
                # user_scale > 1 → wider crop window → boxes appear smaller (zoom out)
                sx = sx_base * scale
                sy = sy_base * scale
                # Crop centre stays at the scaled bbox centre + offset
                cx = (x1 + x2) / 2.0 * sx + dx
                cy = (y1 + y2) / 2.0 * sy + dy
                half_w = (x2 - x1) / 2.0 * sx
                half_h = (y2 - y1) / 2.0 * sy
                ...
                ix2 = min(wi, int(cx + half_w))
                iy2 = min(hi, int(cy + half_h))
                ix2 = max(ix1 + 1, ix2)
                iy2 = max(iy1 + 1, iy2)
                c = iph_img.crop((ix1, iy1, ix2, iy2))
                tmp2 = Path.cwd() / "_tmp_iphone_crop_preview.jpg"
                c.save(str(tmp2), "JPEG", quality=85)
                iphone_lbl.setPixmap(_QP(str(tmp2)).scaled(
                    PREVIEW_SIZE, PREVIEW_SIZE, _Qt.KeepAspectRatio, _Qt.SmoothTransformation))
                scale_lbl.setText(f"Scale: {scale:.2f}×")
            ...
            def _set_scale(val):
                """Called by slider; val is integer 50..300 representing 0.50..3.00."""
                nonlocal scale
                scale = val / 100.0
                _update_iphone()
            ...
            # ── Dialog ────────────────────────────────────────────────────────
            dlg = QDialog(self)
            dlg.setWindowTitle("iPhone Calibration  —  drag right image · adjust scale until boxes match")
            dlg.setMinimumSize(PREVIEW_SIZE * 2 + 80, PREVIEW_SIZE + 220)
            ...
            scan_lbl = _QL()
            scan_lbl.setAlignment(_Qt.AlignCenter)
            scan_lbl.setFixedSize(PREVIEW_SIZE, PREVIEW_SIZE)
            scan_lbl.setStyleSheet("border:1px solid #555;")
            scan_lbl.setPixmap(_QP(str(tmp_scan)).scaled(
                PREVIEW_SIZE, PREVIEW_SIZE, _Qt.KeepAspectRatio, _Qt.SmoothTransformation))

            iphone_lbl = _DraggableLabel(_nudge, _get_scale)
            iphone_lbl.setFixedSize(PREVIEW_SIZE, PREVIEW_SIZE)
            iphone_lbl.setStyleSheet("border:1px solid #777; background:#111;")
            _update_iphone()

            scan_col = QVBoxLayout()
            scan_cap = _QL("◀  DroidCam crop  (reference, fixed)")
            scan_cap.setAlignment(_Qt.AlignCenter)
            scan_col.addWidget(scan_lbl)
            scan_col.addWidget(scan_cap)

            iph_col = QVBoxLayout()
            iph_cap = _QL("▶  iPhone crop  —  drag to pan")
            iph_cap.setAlignment(_Qt.AlignCenter)
            iph_col.addWidget(iphone_lbl)
            iph_col.addWidget(iph_cap)

            imgs.addLayout(scan_col)
            imgs.addLayout(iph_col)

            # ── Scale control ──────────────────────────────────────────────────
            scale_row = QHBoxLayout()
            scale_lbl = _QL(f"Scale: {scale:.2f}×")
            scale_lbl.setFixedWidth(90)
            scale_tip = _QL("← smaller boxes (zoom out)  |  larger boxes (zoom in) →")
            scale_tip.setStyleSheet("font-size:10px; color:#888;")
            slider = QSlider(_Qt.Horizontal)
            slider.setRange(50, 300)          # 0.50× to 3.00×
            slider.setValue(int(round(scale * 100)))
            slider.setTickInterval(10)
            slider.setSingleStep(1)
            slider.setPageStep(10)
            slider.setToolTip("Adjust zoom of the iPhone crop window\n"
                              "Move LEFT to zoom out (make iPhone boxes look smaller)\n"
                              "Move RIGHT to zoom in (make iPhone boxes look bigger)\n"
                              "Goal: boxes in both panels the same size")
            slider.valueChanged.connect(_set_scale)
            scale_row.addWidget(scale_lbl)
            scale_row.addWidget(slider, 1)
            scale_row.addWidget(scale_tip)
            layout.addLayout(scale_row)

            # ── Hint ──────────────────────────────────────────────────────────
            hint = _QL("Drag right image to pan  •  Scroll = ±1 px vertical  •  Shift+Scroll = horizontal  •  Arrows = ±5 px")
            hint.setAlignment(_Qt.AlignCenter)
            ...
            for name in ["↑", "↓", "←", "→", "Reset scale", "OK", "Cancel"]:
                b = QPushButton(name)
                btns.addWidget(b)
                if name == "OK":
                    ...
                    b.clicked.connect(lambda: _nudge(0, -5))
                elif name == "↓":
                    b.clicked.connect(lambda: _nudge(0, 5))
                elif name == "←":
                    b.clicked.connect(lambda: _nudge(-5, 0))
                elif name == "→":
                    b.clicked.connect(lambda: _nudge(5, 0))
                elif name == "Reset scale":
                    b.clicked.connect(lambda: slider.setValue(100))

            if dlg.exec_() == QDialog.Accepted:
                self._iphone_dx    = int(dx)
                self._iphone_dy    = int(dy)
                self._iphone_scale = round(scale, 4)
    ...
    def _iphone_recheck_row_fixed_iph(self, rd, iph_path: str):
        ...
        def _work():
            try:
                ...
                usc   = getattr(self, "_iphone_scale", 1.0)   # user zoom factor
                # Effective per-axis scale = base_scale * user_scale
                esx = sx * usc
                esy = sy * usc
                # Centre-based crop so panning stays intuitive
                cx = (x1 + x2) / 2.0 * esx + dx
                cy = (y1 + y2) / 2.0 * esy + dy
                half_w = (x2 - x1) / 2.0 * esx
                half_h = (y2 - y1) / 2.0 * esy
                ix1 = max(0, int(cx - half_w))
                iy1 = max(0, int(cy - half_h))
                ix2 = min(wi, int(cx + half_w))
                iy2 = min(hi, int(cy + half_h))
    ...
    def _add_result_row(self, r):
        ...
        cl.setToolTip("Left-click on preview → set background color\nRight-click on preview → set shadow color")
        ...
        def make_ctx(rd_ref_):
            def show_ctx(pos):
                ...
                menu.addSeparator()
                a_rescan      = menu.addAction("🎨  Split by color zones (force 2+ results)")
                a_rescan_wide = menu.addAction("🪓  Re-scan wider crop (watershed)")
                ...
                if chosen == a_pg:
                    ...
                    self._rescan_region(rd, expand=0.3, use_watershed=True)
                elif chosen == a_iph:
    ...
    # ------------------------------------------------------------------ #
    #  Color-zone split — segment crop into N dominant-colour regions and  #
    #  query Brickognize for each; insert new results after the source row. #
    # ------------------------------------------------------------------ #
    def _split_attempt(self, rd, n_colors: int = 3, min_area_pct: float = 0.05):
        """
        Segment the crop image into `n_colors` dominant-colour zones using
        K-Means, crop a tight bounding-box for each zone, POST each bbox crop
        to Brickognize concurrently, then insert any NEW (non-duplicate) hits
        after the original row.  The original row is NOT removed.
        """
        # ── 1. resolve the crop image ──────────────────────────────────
        crop_path = rd.get("crop_image", "")
        ...
        if crop_path and Path(crop_path).exists():
            img_path = crop_path
        elif source and Path(source).exists() and bbox:
            # crop from source around bbox
            try:
                from PIL import Image as _PIL
                src_img = _PIL.open(source)
                x1, y1, x2, y2 = bbox
                w_, h_ = src_img.size
                pad = 20
                region = src_img.crop((max(0,x1-pad), max(0,y1-pad),
                                       min(w_,x2+pad), min(h_,y2+pad)))
                import tempfile
                _t = tempfile.NamedTemporaryFile(suffix=".jpg", prefix="split-src-", delete=False)
                region.save(_t.name, quality=95); _t.close()
                img_path = _t.name
            except Exception as e:
                self._log(f"⚠  Split: crop from source failed: {e}", "warning"); return

        if not img_path:
            self._log("⚠  Split by color: no crop image available — scan a part first", "warning")
            return

        # ── 2. find row position for insertion ─────────────────────────
        try:
        ...
        pid_src  = rd.get("part_id", "?")
        self._log(f"🎨  Split by color zones ({n_colors} clusters) for {pid_src}…", "info")
        ...
        def run_split():
            try:
                ...
                img_pil = _PIL2.open(img_path).convert("RGB")
                arr     = np.array(img_pil, dtype=np.float32)
                h, w, _ = arr.shape
                total   = h * w

                # Flatten to pixel list for K-Means
                pixels = arr.reshape(-1, 3)

                # Simple iterative K-Means (avoids sklearn dependency)
                np.random.seed(42)
                ...
                # ── 3. for each cluster ≥ min_area_pct, get bbox crop ──
                import io, requests as _req, tempfile as _tmp

                zone_crops = []   # list of (cluster_idx, PIL image, tmp_path)
                for k in range(n_colors):
                    mask = (labels_2d == k)
                    count = mask.sum()
                    if count / total < min_area_pct:
                        continue   # too small — skip
                    ys, xs = np.where(mask)
                    x1z, x2z = int(xs.min()), int(xs.max())
                    y1z, y2z = int(ys.min()), int(ys.max())
                    # add small padding
                    pad = 4
                    x1z = max(0, x1z - pad)
                    y1z = max(0, y1z - pad)
                    x2z = min(w, x2z + pad)
                    y2z = min(h, y2z + pad)
                    zone_pil = img_pil.crop((x1z, y1z, x2z, y2z))
                    t2 = _tmp.NamedTemporaryFile(suffix=".jpg", prefix=f"split-z{k}-", delete=False)
                    zone_pil.save(t2.name, quality=92); t2.close()
                    zone_crops.append((k, zone_pil, t2.name))

                if not zone_crops:
                    self.log_message.emit("🎨  Split: no usable colour zones found", "info")
                    return

                self.log_message.emit(
                    f"🎨  Split: querying Brickognize for {len(zone_crops)} colour zone(s)…", "info"
                )

                # ── 4. query Brickognize concurrently for each zone ────
                import concurrent.futures

                def _query_zone(zone_path):
                    with open(zone_path, "rb") as fz:
                        raw = fz.read()
                    buf = io.BytesIO(raw)
                    resp = _req.post(
                        "https://api.brickognize.com/predict/",
                        files={"query_image": (Path(zone_path).name, buf.getvalue(), "image/jpeg")},
                        headers={"accept": "application/json"},
                        timeout=30,
                    )
                    if resp.status_code != 200:
                    ...
                    best = items[0]
                    best["_alts"]       = items[1:6]
                    best["_zone_path"]  = zone_path
                    return best

                results = []
                with concurrent.futures.ThreadPoolExecutor(max_workers=len(zone_crops)) as pool:
                    futs = {pool.submit(_query_zone, zp): zp for _, _, zp in zone_crops}
                ...
                if not results:
                    self.log_message.emit("🎨  Split: Brickognize returned no matches", "info")
                    return

                self.log_message.emit(
                    f"🎨  Split: {len(results)} match(es) from colour zones", "success"
                )
                self._rescan_ready.emit(rd, self._split_hits_to_entries(results, rd))

            except Exception as ex:
                import traceback
                self.log_message.emit(f"⚠  Split error: {ex}", "warning")

        # Store insertion position (shared with _on_rescan_ready)
    ...
    def _split_hits_to_entries(self, hits, source_rd):
        """
        Convert Brickognize hit dicts (from _split_attempt) into the same
        entry format that _on_rescan_ready expects, deduplicating within hits.
        """
        seen_ids = set()
        entries  = []
        for hit in hits:
            ...
            entry = {
                "part_id":      pid,
                "part_name":    hit.get("name", ""),
                "confidence":   float(hit.get("score", 0) or 0),
                "item_type":    hit.get("type", "P"),
                "thumb_url":    hit.get("img_url", ""),
                "crop_image":   hit.get("_zone_path", source_rd.get("crop_image", "")),
                "source_image": source_rd.get("source_image", ""),
                "bbox":         source_rd.get("bbox"),
                "color_id":     0,
                "color_name":   "",
                "_split_alts":  hit.get("_alts", []),
            }
    ...
    def _bulk_pixel_recolor(self):
        ...
        def _sample_rgb(crop_path):
            ...
            try:
                ...
                def clean(region):
                    ...
                    not_black    = ~(mx < 18)   # lowered from 25 — keeps valid very-dark pixels
        ...
        for rd in sel_rows:
            ...
            # Dark-part guard: exclude specular pixels from median measurement;
            # raise threshold to 80 to catch glossy-black parts.
            try:
                arr_chk = np.array(_PIL_Image.open(crop_path).convert("RGB").resize((60,60)))
                br_ch = arr_chk.max(axis=2).astype(float)
                non_spec = br_ch[br_ch < 210]
                med_br = float(np.median(non_spec)) if len(non_spec) > 50 else float(np.median(br_ch))
            except Exception:
                med_br = 128.0
            dark_part = med_br < 80 or max(sampled) < 70

            # Also force dark pool if sampled RGB itself is very dark
            if max(sampled) < 55:
                dark_part = True
            ...
            # L-weighted Lab distance: for near-black samples, luminance matters
            # far more than a/b chroma — warm/cool light tints should not flip the result.
            _wL = 3.0 if s_lab[0] < 30 else 1.0
            _wC = 0.4 if s_lab[0] < 30 else 1.0
            best_cid = best_name = best_rgb = None
            best_dist = float("inf")
            for cid, name, rgb in candidates:
                ref = lab_cache.get(cid)
                if ref is None: continue
                dL = s_lab[0]-ref[0]; da = s_lab[1]-ref[1]; db = s_lab[2]-ref[2]
