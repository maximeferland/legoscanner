...
def _sample_part_color_rgb_inner(crop_img):
    ...
    def _clean_pixels(region):
        ...
        # Exclude pure black shadows (keep dark part pixels — threshold raised to 25)
        not_black = ~((brightness < 25))
...
def resolve_color_from_cache(part_id: str, crop_img, brightness_bias: int = 0):
    ...
    _median_brightness = float(np.median(_arr_full.max(axis=2)))  # max channel = brightness
    _dark_part = _median_brightness < 60  # overall image is dark → part is likely dark
    _lum_filter = set()
    if _dark_part:
        _lum_filter = {cid for cid, lab in _BL_COLORS_LAB.items() if lab[0] < 40}
        print(f"   🎨  Dark image (med brightness {_median_brightness:.0f}) — restricting to {len(_lum_filter)} dark colors")
