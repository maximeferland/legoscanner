...
class ScanStation(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.iphone_dir_btn = self._btn("Set…", CARD_BG, self._set_iphone_photo_dir, w=58)
        ...
        gl.addWidget(self.grid_check, 12, 0, 1, 2)
        grid_sub = QWidget()
        ...
        gl.addWidget(grid_sub, 13, 0, 1, 2)
    ...
    def _iphone_recheck_row_fixed_iph(self, rd, iph_path: str):
        ...
        def _work():
            try:
                ...
                sx = wi / float(wd)
                sy = hi / float(hd)
                ix1 = max(0, min(int(x1 * sx), wi - 2))
                ix2 = max(ix1 + 1, min(int(x2 * sx), wi))
                iy1 = max(0, min(int(y1 * sy), hi - 2))
                iy2 = max(iy1 + 1, min(int(y2 * sy), hi))
