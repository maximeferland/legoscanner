...
from PyQt5.QtWidgets import (
    ...
    QDialogButtonBox, QLineEdit, QFileDialog, QMenu
)
from PyQt5.QtCore import Qt, QThread, pyqtSignal, QTimer
from PyQt5.QtGui import QColor, QPalette, QPixmap, QTextCursor, QImage
...
class ScanWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Grab frame directly from the warm live camera if available
            scan_path = None
            ...
            if s.get("forced_image_path"):
            ...
            elif self.main_win._last_frame is not None and (self.main_win._live_running or self.main_win._stream_active):
                import cv2
                from datetime import datetime
                ...
                frame = self.main_win._last_frame
...
class ScanStation(QMainWindow):
    ...
    def _build_ui(self):
        ...
        gl.addWidget(QLabel("IP stream:"), 1, 0)
        ip_w = QWidget(); ip_h = QHBoxLayout(ip_w)
        ip_h.setContentsMargins(0,0,0,0); ip_h.setSpacing(4)
        self.stream_url = QComboBox(); self.stream_url.setEditable(True)
        self.stream_url.lineEdit().setPlaceholderText("192.168.x.x:4747")
        ...
        self.webcam_btn.setToolTip("Switch to local webcam")
        ip_h.addWidget(self.stream_url); ip_h.addWidget(self.use_stream_btn); ip_h.addWidget(self.webcam_btn)
        gl.addWidget(ip_w, 1, 1)

        # Confidence
        gl.addWidget(QLabel("Confidence:"), 2, 0)
        ...
        gl.addWidget(cfw, 2, 1)

        # Currency
        gl.addWidget(QLabel("Currency:"), 3, 0)
        self.currency_combo = QComboBox()
        self.currency_combo.addItems(["CAD","USD","EUR","GBP"])
        gl.addWidget(self.currency_combo, 3, 1)

        # Force color
        gl.addWidget(QLabel("Force color:"), 4, 0)
        self.color_combo = QComboBox()
        for name, val in COLORS:
            self.color_combo.addItem(name, val)
        gl.addWidget(self.color_combo, 4, 1)

        # Qty
        gl.addWidget(QLabel("Qty per part:"), 5, 0)
        self.qty_spin = QSpinBox()
        self.qty_spin.setRange(1,99); self.qty_spin.setValue(1)
        gl.addWidget(self.qty_spin, 5, 1)

        gl.addWidget(QLabel("Blob gap:"), 6, 0)
        ...
        gl.addWidget(self.gap_spin, 6, 1)

        gl.addWidget(QLabel("Detection:"), 7, 0)
        ...
        gl.addWidget(det_w, 7, 1)

        gl.addWidget(QLabel("Crop padding:"), 8, 0)
        ...
        gl.addWidget(self.padding_spin, 8, 1)

        gl.addWidget(QLabel("Color bias:"), 9, 0)
        bias_w = QWidget()
        ...
        self.brightness_bias.setRange(-60, 60); self.brightness_bias.setValue(30)
        ...
        gl.addWidget(grid_sub, 11, 0, 1, 2)
        ...
        gl.addWidget(QLabel("Background:"), 12, 0)
        bg_sub = QWidget()
        ...
        tog_hl.addWidget(self.live_btn); tog_hl.addWidget(self.snap_btn); tog_hl.addWidget(self.enlarge_btn); tog_hl.addStretch()
        pl.addWidget(tog_h)
        lv.addWidget(pg)
        ...
        # Row 2: XML · Copy · Report · Merge Dupes
        ...
        for b in [self.copy_btn, self.clip_btn, self.html_btn, self.merge_r2]:
        ...
        for b in [self.copy_btn, self.clip_btn, self.html_btn,
                  self.merge_r2, self.merge_btn, self.upload_btn, self.bs_btn]:
            b.setEnabled(False)
        ...
        self._bulk_agg      = self._btn("Σ Merge",  CARD_BG, self._bulk_aggregate_selected,  w=91)
        ...
        self._bulk_agg.setToolTip("Merge selected rows with same part+color, summing quantities")
        ...
        _all_bulk_btns = [self._bulk_sel_all, self._bulk_sel_none, self._bulk_set_qty,
                          self._bulk_set_med, self._bulk_set_med15, self._bulk_price,
                          self._bulk_color, self._bulk_bg_color, self._bulk_used, self._bulk_new,
                          self._bulk_remark, self._bulk_del, self._bulk_agg, self._bulk_undo]
        ...
        self._undo_snapshot = None  # single-level undo: {"action", "rows_snapshot"}
        ...
        self.total_value_label.setAlignment(Qt.AlignRight)
    ...
    def _start_live_camera(self):
        ...
        def delayed_start():
            ...
            # Read camera index from station.cfg first (most reliable),
            # fall back to combo box if cfg not available
            cam_idx = self._get_active_cam_idx()
            if CFG_PATH.exists():
                try:
                    _cfg_cam = json.loads(CFG_PATH.read_text()).get("camera")
                    if _cfg_cam is not None:
                        cam_idx = int(_cfg_cam)
                except Exception:
                    pass
            ...
            try:
                ...
                for _ in range(30):
                    ...
                    self.log_message.emit(f"⚠  Camera {cam_idx} — no frame received", "warning")
    ...
    def _run_calibrate(self):
        ...
        # Snapshot stream info BEFORE stopping (stop clears _use_http_stream)
        stream_url_snapshot = (self._http_stream_url
                               if self._use_http_stream and self._http_stream_url
                               else "")
        ...
        if stream_url_snapshot:
            cmd = [_python_exe(), "calibrate-station.py", "--stream", stream_url_snapshot]
            self._log(f"    Using HTTP stream: {stream_url_snapshot}", "info")
        else:
            # Read camera index from station.cfg (most reliable), fallback to combo
            cam_idx = 0
            try:
                if CFG_PATH.exists():
                    _c = json.loads(CFG_PATH.read_text()).get("camera")
                    if _c is not None:
                        cam_idx = int(_c)
                else:
                    combo_idx = self.camera_combo.currentData()
                    if combo_idx is not None and combo_idx >= 0:
                        cam_idx = combo_idx
            except Exception:
                pass
            cmd = [_python_exe(), "calibrate-station.py", "--camera", str(cam_idx)]
            self._log(f"    Using camera index: {cam_idx}", "info")

        try:
            proc = subprocess.Popen(cmd, env=env, cwd=str(Path.cwd()),
                                     **_hidden_popen_kwargs())
        except Exception as e:
            self._log(f"⚠  Could not launch calibration: {e}", "error")
            return
    ...
    def _fetch_price_for_row(self, rd):
        ...
        def fetch_and_emit():
            """Runs in background thread — reads color_id fresh from rd at call time."""
            color_id = rd.get("color_id", 0)   # read inside thread so it's current
            ...
            try:
                ...
                rd["item_type"] = itype   # correct it in-place for URL/XML too
                if itype == "M":
                ...
                if r.status_code == 404 and itype == "M":
                    # Minifig endpoint 404 — retry as part
                    url2    = f"https://api.bricklink.com/api/store/v1/items/part/{pid}/price"
                    params2 = {"guide_type": "sold", "new_or_used": "U",
                               "currency_code": currency, "color_id": color_id}
                    r2 = req.get(url2, params=params2, auth=auth, timeout=8)
                    if r2.status_code == 200:
                        rd["item_type"] = "P"
                        r = r2
                    else:
                        # Not found anywhere — silently skip, show — in price
                        self._price_ready.emit(rd, None); return
                elif r.status_code == 404:
                    # Part not found — silently skip
                    self._price_ready.emit(rd, None); return
                elif r.status_code != 200:
                    self.log_message.emit(
                        f"⚠  Price API {r.status_code} for {pid}: {r.text[:80]}", "warning")
                    self._price_ready.emit(rd, None); return
                ...
                import traceback
                self.log_message.emit(f"⚠  Price fetch ({pid}): {e}\n{traceback.format_exc()}", "warning")
    ...
    def _add_result_row(self, r):
        ...
        cwh.addWidget(sw); cwh.addWidget(cnl); rl.addWidget(cw)
        ...
        if r.get("alternatives"):
            ...
            if not has_mv and r.get("part_id"):
                ...
                if _re2.match(r"^[0-9]+[a-z]?(pb|pr|pat|ps)", r["part_id"].lower()):
                    has_mv = True
            btn_text = "▾ 🔀" if has_mv else "▾ alt"
            btn_tip  = "Mold variants available — click to see BrickLink alternatives" if has_mv else "Pick alternative Brickognize result"
            btn_col  = WARNING if has_mv else ACCENT2
            alt_btn = _QL(btn_text); alt_btn.setAlignment(Qt.AlignCenter)
            alt_btn.setFixedHeight(14)
            alt_btn.setStyleSheet(f"color:{btn_col};font-size:9px;text-decoration:underline;font-weight:{'bold' if has_mv else 'normal'};")
        ...
        # Snapshot the Brickognize scan color so it can always be restored
        row_data["_scan_color_id"]   = r.get("color_id")
        row_data["_scan_color_name"] = r.get("color_name")
        row_data["_scan_color_rgb"]  = r.get("color_rgb")
        row_data.update({
            "_widget":    row,
            "_qty_lbl":   qtyl,
            "_swatch":    sw,
        ...
        def make_ctx(rd_ref_):
            def show_ctx(pos):
                ...
                menu.addSeparator()
                a_rescan      = menu.addAction("🔍  Re-scan this area (split merged parts)")
                a_rescan_wide = menu.addAction("🔍+  Re-scan wider area +50% (part cut off)")
    ...
    def _save_undo_snapshot(self, action=""):
        """Save mutable field values for all rows — no widget refs."""
        self._undo_snapshot = {
            "action": action,
        ...
        tip = f"↩ Undo: {action}" if action else "↩ Undo"
    ...
    def _undo_last(self):
        ...
        self._log(f"↩  Undid: {snap['action']}", "info")
    ...
    def _show_sessions(self):
        ...
        for d in sorted(reports_dir.iterdir(), reverse=True):
            ...
            try:
                ...
                total = sum(
                    (r.get("price") or r.get("medium_price") or 0) * r.get("qty", 1)
                    for r in data
                )
    ...
    def _rescan_region(self, rd, expand=0.0):
        """Re-scan a box region. expand=0.5 makes the crop 50% wider/taller on each side."""
        ...
        label = f"🔍+  Re-scanning wider area for {rd.get('part_id','?')}" if expand > 0 else f"🔍  Re-scanning region for {rd.get('part_id','?')}"
    ...
    def _update_total_value(self):
        """Recompute and display total value of selected rows (or all if none selected)."""
        sel = self._selected_rows()
        rows = sel if sel else [r for r in self._rows if not r.get("_deleted") and r.get("part_id")]
        total = sum((r.get("price") or 0) * r.get("qty", 1) for r in rows)
        n_lots = len(rows)
        n_qty  = sum(r.get("qty", 1) for r in rows)
        currency = self.currency_combo.currentText() if hasattr(self, "currency_combo") else "CAD"
        if not rows:
            self.total_value_label.setText("")
        elif sel:
            self.total_value_label.setText(
                f"Selected: {n_lots} lots · {n_qty} pcs · ${total:.2f} {currency}")
        else:
            self.total_value_label.setText(
                f"Total: {n_lots} lots · {n_qty} pcs · ${total:.2f} {currency}")
    ...
    def _load_batch_files(self):
        ...
        paths, _ = QFileDialog.getOpenFileNames(
            self, "Select images to batch scan", "",
            "Images (*.jpg *.jpeg *.png *.bmp *.tiff *.webp)")
        if not paths:
            return
        paths = sorted(paths)
